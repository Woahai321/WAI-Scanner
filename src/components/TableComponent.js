...
const SaveBtn = ({ data }) => {
  ...
  const storeCoin = (e, id) => {
    saveCoin(id);
    // e.target.style.fill = "#14ffec"
  };

  const deleteCoin = (e, id) => {
    removeCoin(id);
    // e.target.style.fill = "#808080"
  };

  const handleClick = (e, coinId) => {
    if (allCoins.includes(coinId)) {
      deleteCoin(e, coinId);
    } else {
      storeCoin(e, coinId);
    }
...
const TableComponent = () => {
  const { cryptoData, currency, error, loading } =
  ...
  return (
    <>
      <div className="flex flex-col mt-9 border border-gray-100 rounded  ">
                ...
                cryptoData.map((data) => {
                  return (
                    ...
                    >
                      ...
                      <td
                        className="py-4 cursor-pointer sm:table-cell hidden"
                      >
                        <Link to={`/${data.id}`}
                          className="cursor-pointer"
                        >
        ...
        ) : (!error.data || !error.search || loading) ? (
